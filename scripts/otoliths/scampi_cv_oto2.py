...
import numpy as np
import ctfishpy
from ctfishpy.train_utils import Trainer, test_otoliths, CTDataset, precache
...
def train(config, dataset_path, name, bone, train_data, val_data, test_data, save=False, tuner=True, device_ids=[0,1], num_workers=10, work_dir="."):
	...
	gc.collect()
	losses = test_otoliths(dataset_path, model, bone, test_data, params, threshold=0.5, run=run, criterion=criterion, device=device, num_workers=num_workers, label_size=label_size)
...
if __name__ == "__main__":
	...
	val_data = all_keys[18:23]
	test_data = all_keys[23:]
	# train_data = all_keys[1:2]
	# val_data = all_keys[2:3]
	# test_data = all_keys[3:4]

	print(f"train = {train_data} len {len(train_data)} \n val = {val_data} len {len(val_data)} \n test = {test_data} len {len(test_data)} \n")
	name = 'oto_cv_test'
	save = False
	...
	fold_n = 0
	for i in range(0,n_all_keys,fold_size):
		fold = all_keys[i:i+fold_size]
		print(f"fold {fold_n+1} len {len(fold)} test {fold}")
		name = f'oto cv{fold_n+1}'
		fold_n+=1

  
		train_data = [x for x in all_keys if x not in fold]
		val_data = fold
		test_data = fold
